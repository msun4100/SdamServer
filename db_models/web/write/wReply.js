...
var writeReply = function (datas, callback) {
	pool.getConnection(function (err, conn) {
		if(err) {
			...
			conn.query(sql, [datas[0], datas[2], datas[3], datas[4]], function (err, rows) {
				if(err) {
					// query error
	...
	pool.getConnection(function (err, conn) {
		if(err) {
			...
			if(typeof(tagIds) === "string") {
				tagIds = [];
				tagIds.push(datas[5]);
			} else if(typeof(tagNicks) === "string") {
				tagNicks = [];
			...
			if(tagIds !== null && tagNicks !== null) {
				...
				// each는 배열의 길이를 확인하여 그만큼 작동, 현재 tagIds가 배열이 아니기에 문제
...
// datas :  [ '630', 'winnersky7@naver.com', 'server@', 'ㅁㅁ', '34', '', '' ]
...
exports.wReply=function (datas, callback) {
	async.waterfall([
		function (callback) {
...
// 		var temp = [repData[0].good_count, repData[0].regDate];
...
// 				temp[1] = repData[i].regDate;
// 				bRindex = i;	// 댓글인덱스 저장
// 			}
// 		} else if(temp[0] == repData[i].good_count) {
// 			// 현재 좋아요 값과 같을 경우
// 			if(temp[1] == repData[i].regDate) {
