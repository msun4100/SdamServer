...
var articleCount = function (page, callback) {
	pool.getConnection(function (err, conn) {
		if(err) {
			...
			var sql = "SELECT COUNT(*) AS cnt FROM board WHERE notice='N'";
...
var getList = function (datas, callback) {
	pool.getConnection(function (err, conn) {
		if(err) {
			...
			var sql = "SELECT num, writer, content, DATE_FORMAT(reg_date, '%Y-%c-%d %H:%i:%s') AS regDate, click_num AS hit, good_count AS goodCount, reply_count AS replyCount FROM all_view WHERE notice='N' ORDER BY num DESC LIMIT ?, ?";
