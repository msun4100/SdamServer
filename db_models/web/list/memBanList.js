...
var banCount = function (page, callback) {
	pool.getConnection(function (err, conn) {
		if(err) {
			var error = 'DB_connection_error';
			callback(err, error);
		} else {
			var sql = "SELECT COUNT(*) AS cnt FROM member WHERE ban=1";
			conn.query(sql, [], function (err, rows) {
				if(err) {
					var error = 'banCount_error';
					conn.release();
					callback(err, error);
				} else {
...
// 글목록 불러오기
var getBanList = function (datas, callback) {
	pool.getConnection(function (err, conn) {
		if(err) {
			var error = 'DB_connection_error';
			callback(err, error);
		} else {
			var sql = "SELECT email, birth, sex, ban_date AS banDate FROM member WHERE ban=1 LIMIT ?, ?";
			conn.query(sql, [datas.begin, datas.size], function (err, list) {
				if(err) {
					var error = 'getBanList_error';
					conn.release();
					callback(err, error);
				} else {
					// query 성공
...
exports.memBanList = function (page, callback) {
	async.waterfall([
		function (callback) {
