...
var insertBan = function (datas, callback) {
	pool.getConnection(function (err, conn) {
		if(err) {
			...
			// DB 연결
			var sql = 'INSERT INTO article_ban(id, num, writer, ban_date) VALUES(?, ?, ?, now())';
...
var countBan = function (datas, callback) {
	pool.getConnection(function (err, conn) {
		if(err) {
			...
			// DB연결
			var sql = 'SELECT COUNT(*) AS cnt FROM article_ban WHERE writer=? AND ban_date > DATE_ADD(NOW(), INTERVAL -3 MONTH)';
...
exports.articleBan = function (datas, callback) {
	async.waterfall([
		function (callback) {
