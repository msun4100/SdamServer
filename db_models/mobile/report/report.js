...
var insertBan = function (datas, callback) {
	pool.getConnection(function (err, conn) {
		if(err) {
			...
			// DB 연결
			var sql = 'INSERT INTO report(num, writer, category, report_date) VALUES(?, ?, ?, now())';
...
var countBan = function (datas, callback) {
	pool.getConnection(function (err, conn) {
		if(err) {
			...
			// DB연결
			var sql = 'SELECT COUNT(*) AS cnt FROM report WHERE writer=? AND report_date > DATE_ADD(NOW(), INTERVAL -3 MONTH)';
			conn.query(sql, datas[1], function (err, count) {
				if(err) {
	...
	pool.getConnection(function (err, conn) {
		if(err) {
			...
			if(countData >= 5) {
				...
				conn.query(sql, datas[1], function (err, result) {
					if(err) {
